...
async def ask_remove_protect(message: types.Message, state: FSMContext):
    """Ask for number to remove from protection"""
    if CHANNEL_ID and not await check_subscription(message.from_user.id):
        return
...
async def process_remove_protect(message: types.Message, state: FSMContext):
    """Process removal from protection"""
    number = clean_phone_number(message.text.strip())
...
if user_id in PROTECTED_NUMBERS and number in PROTECTED_NUMBERS[user_id]:
    ...
    await message.answer("❌ This number is not protected by you.")
        await message.answer(
...
async def protected_numbers(message: types.Message):
    user_id = message.from_user.id
...
async def all_protected(message: types.Message):
    ...
    if not PROTECTED_NUMBERS:
        await message.answer("❌ No protected numbers.")
        return
